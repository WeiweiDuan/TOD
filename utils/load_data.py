...
def load_wetland_samples(target_data_path,img_size=28,channel=1):
    x_train, f_name = [], []
    for root, directory, files in os.walk(target_data_path):
        for fname in files:
            if '.png' not in fname:# or cv2.imread(os.path.join(root, fname)).shape!=(img_size,img_size,3):
                continue
            x_train.append(cv2.imread(os.path.join(root, fname),0))
